...
for t=0:timestep:timeend
    ...
    % 5. calculate u_e^\kappa (修改后的逃避者策略)
    for i = 1:length(evaders)
        if evaders{i}.isDead
            continue;
        end
        % 仅向Voronoi单元质心移动，忽略追捕者威胁
        centroid = mean(evaders{i}.voronoi_cell.V, 1);
        dir = centroid - evaders{i}.position;
        if norm(dir) > 0
            dir = dir / norm(dir);
        else
            dir = [0, 0];
        ...
        % 设置速度为最大速度乘以方向向量
        evaders{i}.velocity = evaders{i}.max_speed * dir;
    end
